...
public class Schedule<R, A, State, B> {
    ...
    public func pipe<State2, C>(_ that: Schedule<R, B, State2, C>) -> Schedule<R, A, (State, State2), C> {
        Schedule<R, A, (State, State2), C>(
            ...
            update: { a, s in
                self.update(a, s.0).flatMap { s1 in
                    that.update(self.extract(a, s.0), s.1).map { s2 in
                        (s1, s2)
                    }
                }
            }
    ...
    public func addDelayM(_ f: @escaping (B) -> EnvIO<R, Never, TimeInterval>) -> Schedule<R, A, State, B> {
        updated { upd in
            { a, s in
                f(self.extract(a, s)).unitError().flatMap { delay in
                    upd(a, s).flatMap { s1 in
                        EnvIO.sleep(delay).map { _ in s1 }
                    }
                }
            }
        }
...
extension EnvIO {
    func unitError() -> EnvIO<Dependencies, Unit, Success> {
        mapError(constant(.unit))
    }
}
